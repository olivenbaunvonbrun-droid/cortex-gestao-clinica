...
function formatClinicalContent(markdown: string): string {
  // Deep cleaning and clean HTML conversion
  return markdown
    // Remove triple hashes and headers
...
export function exportToHtml(assessment: Assessment) {
  ...
  const fileName = `${assessment.patient.name.replace(/\s+/g, '_')}_IHS_Digital_${formattedDate}_${formattedTime}.html`;
...
<head>
    ...
    <title>Relatório IHS - ${assessment.patient.name}</title>
...
<body>
    <div class="paper">
        <div class="watermark">IHS DIGITAL</div>
        
        <header>
            ${assessment.patient.logoUrl ? `<img src="${assessment.patient.logoUrl}" class="header-logo" alt="Logo Profissional">` : `<div class="logo-box">IHS DIGITAL</div>`}
            <h1 class="main-title">Inventário de Habilidades Sociais (IHS-2)</h1>
        </header>
        ...
        <footer>
            ...
            <div class="legal-notice">
                <strong>REFERÊNCIAS E RESPONSABILIDADES:</strong> O presente documento foi elaborado estritamente sob as diretrizes da Resolução CFP nº 06/2019 do Conselho Federal de Psicologia. Trata-se de documento sigiloso cuja guarda é de responsabilidade do profissional e do paciente. O Inventário de Habilidades Sociais (IHS-Del-Prette) é um instrumento validado cientificamente (Del Prette & Del Prette, 2001) e sua interpretação deve ser feita exclusivamente por profissional habilitado.
            </div>
        </footer>
    </div>
