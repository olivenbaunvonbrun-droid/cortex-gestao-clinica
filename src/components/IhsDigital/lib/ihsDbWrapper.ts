...
export function formatIhsToHtml(assessment: Assessment): string {
  return `
    <div class="ihs-entry-rendered p-6 bg-white/[0.01] border border-white/[0.06] rounded-2xl space-y-4">
      <div class="flex items-center justify-between border-b border-white/[0.08] pb-3 mb-3">
        <h4 class="text-xs font-black uppercase tracking-wider text-[#4dabf7]">Inventário de Habilidades Sociais (IHS)</h4>
      ...
      <p class="text-xs leading-relaxed opacity-95">
        Avaliação concluída e laudo gerado pela inteligência artificial com base nas 42 respostas fornecidas sobre habilidades sociais (fatores de Enfrentamento, Positividade, Conversação, Autoexposição e Autocontrole).
      </p>
      ${assessment.aiAnalysis ? `
        <div class="mt-4 pt-4 border-t border-white/[0.08]">
          <h5 class="text-xs font-black uppercase text-indigo-400 tracking-wider mb-2">Relatório Clínico e Conclusão</h5>
...
class IhsDatabaseWrapper {
    ...
    if (patientId) {
      ...
      if (record) {
        record.entradas.forEach(e => {
          if (e.tipo === 'ihs' || e.metadata?.type === 'ihs') {
            const ihs = e.metadata?.ihsData;
            if (ihs) assessments.push(ihs);
          }
        });
      }
    } else {
          ...
          if (e.tipo === 'ihs' || e.metadata?.type === 'ihs') {
            ...
            if (ihs) {
              assessments.push({
                ...
                patient: {
                  ...ihs.patient,
                  name: patient?.nome || ihs.patient.name
                }
              });
            }
          }
  ...
  async saveEntry(assessment: Assessment, patientId: string, userId?: string): Promise<Assessment[]> {
    const record = await db.prontuarios.get(patientId);
    const textHtml = formatIhsToHtml(assessment);
...
export const dbWrapper = new IhsDatabaseWrapper();
