...
interface HistoryViewProps {
  records: PciRecord[];
  onView: (r: PciRecord) => void;
  onDelete: (id: string) => void;
}

export function HistoryView({ records, onView, onDelete }: HistoryViewProps) {
  if (records.length === 0) {
    return (
      <div className="text-center py-20 bg-bg-card rounded-[2rem] border border-border-subtle border-dashed">
        ...
        <h3 className="text-xs font-black text-text-main uppercase tracking-widest">Nenhum plano encontrado</h3>
        <p className="text-text-dim text-[9px] mt-2 uppercase font-black tracking-wider">Os planos clínicos e tratamentos estruturados arquivados aparecerão aqui.</p>
      </div>
    );
  }
  ...
  const getPhaseLabel = (phase: string) => {
    switch (phase) {
      case 'triagem': return 'Triagem / Avaliação';
      case 'intervencao': return 'Intervenção Ativa';
      case 'alta': return 'Preparação para Alta';
      case 'recaida': return 'Acompanhamento';
    ...
    >
      <div className="flex items-center justify-between px-2 mb-2 border-b border-border-subtle pb-4">
        <div>
          ...
          <p className="text-[10px] text-text-dim font-bold uppercase tracking-wider mt-1">Gestão de planos clínicos e metas</p>
        </div>
        <div className="text-right">
          <span className="text-[9px] font-black text-text-dim uppercase tracking-widest block">Total de Planos</span>
          <span className="text-2xl font-black text-primary">{records.length}</span>
        </div>
      </div>
        ...
        {records.map((r, idx) => (
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0 pr-2 flex-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[8px] font-black text-text-dim uppercase tracking-widest bg-bg-sidebar border border-border-subtle px-2 py-0.5 rounded-md">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                Abrir Plano Terapêutico <ChevronRight size={14} />
              </button>
              <button 
