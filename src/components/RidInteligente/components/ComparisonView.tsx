...
import { X, GitCompare, Printer } from 'lucide-react';
...
export function ComparisonView({ entries, onClose, settings }: ComparisonViewProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-bg-deep overflow-y-auto animate-in fade-in duration-300 select-none">
      <div className="max-w-7xl mx-auto px-6 py-8">
        <header className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-sm font-black text-text-main uppercase tracking-wider">Comparação Evolutiva</h2>
              <p className="text-[10px] text-text-dim font-bold uppercase tracking-widest mt-0.5">Análise de Progresso Clínica</p>
            </div>
          </div>
          <button 
            onClick={onClose}
            className="p-2 hover:bg-white/5 rounded-full transition-colors text-text-dim hover:text-text-main cursor-pointer"
          >
            <X size={20} />
          </button>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {entries.map((entry, idx) => (
            <div key={entry.id} className="flex flex-col gap-4">
              ...
              <div className="flex-1 bg-bg-card p-6 rounded-2xl border border-border-subtle shadow-sm overflow-hidden flex flex-col">
                <div className="flex justify-between items-center mb-4">
                ...
                <div className="prose prose-invert prose-sm max-w-none text-text-main/80 text-justify prose-p:leading-relaxed overflow-y-auto scroller-hide">
