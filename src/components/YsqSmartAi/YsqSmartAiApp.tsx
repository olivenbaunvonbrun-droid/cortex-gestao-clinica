...
import { YSQ_QUESTIONS, PatientData, Assessment, Frequency, FREQUENCY_LABELS, SCHEMA_DETAILS } from './types';
...
import { dbWrapper } from './lib/ysqDbWrapper';
...
export default function YsqSmartAiApp({ activePatientId, lockPatient = false, userId, onClose }: YsqSmartAiAppProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (Object.keys(answers).length < YSQ_QUESTIONS.length) {
      toast.error(`Por favor, responda todas as ${YSQ_QUESTIONS.length} perguntas.`);
      return;
    }
    ...
    try {
      ...
      // Calculate high schemas for context
      ...
      const activeSchemasText = Object.entries(SCHEMA_DETAILS)
        .map(([key, info]) => {
          const score = (schemaSums[key] || 0) / (schemaCounts[key] || 5);
          return { name: info.name, score };
        })
      ...
      const answersText = YSQ_QUESTIONS.map(q => {
        const freq = answers[q.id] || Frequency.F1;
        return `Item ${q.id}: ${freq} - "${q.text}"`;
      }).join('\n');

      let analysis = '';
      try {
        analysis = await analyzeYsqAssessment({ name: pData.name, age: pData.age }, activeSchemasText, answersText);
      } catch (err: any) {
        console.error("Erro ao gerar análise YSQ via IA:", err);
        toast.error('Não foi possível gerar a análise da IA, mas o questionário foi salvo localmente.');
      ...
      console.error(error);
      toast.error('Erro ao processar e salvar a avaliação.');
    } finally {
      ...
      <header className="h-14 bg-bg-card border-b border-border-subtle px-6 flex items-center justify-between shrink-0 relative z-50">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-sm font-bold tracking-tight text-text-main flex items-center gap-2">
            YSQ
            <span className="text-primary font-black">Smart AI</span> 
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {(patients || []).map(p => (
              <option key={`ysq-patient-opt-${p.id}`} value={p.id} className="bg-bg-card text-text-main">
                {p.nome}
              </option>
            ))}
          </select>
          ...
          {activeTab === 'test' && !currentResult && selectedPatientId && (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={isAnalyzing || Object.keys(answers).length < YSQ_QUESTIONS.length}
                    ...
                    <div className="border-t border-border-subtle pt-6">
                      ...
                      <div className="flex items-center justify-between text-xs p-4 bg-bg-card rounded-2xl border border-border-subtle shadow-inner">
                        ...
                        <span className="font-black text-primary">
                          {Object.keys(answers).length} / {YSQ_QUESTIONS.length}
                        </span>
                      </div>
                      ...
                      <div className="mt-4 space-y-1">
                        <div className="flex justify-between text-[8px] font-black uppercase text-text-dim/60">
                          <span>Progresso</span>
                          <span>{Math.round((Object.keys(answers).length / YSQ_QUESTIONS.length) * 100)}%</span>
                        </div>
                        <div className="h-1.5 bg-bg-sidebar border border-border-subtle rounded-full overflow-hidden">
                          <div 
                            className="h-full bg-primary rounded-full transition-all duration-300"
                            style={{ width: `${(Object.keys(answers).length / YSQ_QUESTIONS.length) * 100}%` }}
                          />
                        </div>
                      </div>
                    </div>
