...
export async function exportToHtml(assessment: Assessment) {
  ...
  const fileName = `${assessment.patient.name.replace(/\s+/g, '_')}_YSQ_SmartAI_${formattedDate}_${formattedTime}.html`;
  ...
  const getScoreBadge = (score: number) => {
    if (score >= 4.0) {
      return `<span style="background-color: #fee2e2; color: #b91c1c; border: 1px solid #fecaca; padding: 3px 8px; border-radius: 6px; font-weight: 800; font-size: 10px; font-family: 'Inter', sans-serif;">ATIVO (ALTO)</span>`;
    } else if (score >= 3.0) {
      return `<span style="background-color: #fef3c7; color: #b45309; border: 1px solid #fde68a; padding: 3px 8px; border-radius: 6px; font-weight: 800; font-size: 10px; font-family: 'Inter', sans-serif;">LATENTE (MÉDIO)</span>`;
    } else {
      return `<span style="background-color: #f0fdf4; color: #15803d; border: 1px solid #dcfce7; padding: 3px 8px; border-radius: 6px; font-weight: 800; font-size: 10px; font-family: 'Inter', sans-serif;">INATIVO (BAIXO)</span>`;
    }
  };

  const tableRows = schemaScores.map((s, idx) => `
    <tr style="background: ${idx % 2 === 0 ? '#ffffff' : '#f8fafc'}; border-bottom: 1px solid #e2e8f0;">
      <td style="padding: 12px 16px; font-weight: 700; color: #0f172a; font-size: 12px;">${s.name}</td>
      <td style="padding: 12px 16px; color: #475569; font-size: 11px; font-weight: 600;">${s.domain}</td>
      <td style="padding: 12px 16px; text-align: center; font-weight: 800; color: #0f172a; font-size: 12px; font-family: monospace;">${s.score.toFixed(1)}</td>
      <td style="padding: 12px 16px; text-align: center;">${getScoreBadge(s.score)}</td>
    </tr>
  `).join('');
...
<head>
    ...
    <title>Relatório YSQ-S3 - ${assessment.patient.name}</title>
...
<body>
    <div class="paper">
        <div class="watermark">YSQ SMART AI</div>
        
        <header>
            ${logoUrl ? `<img src="${logoUrl}" class="header-logo" alt="Logo Profissional">` : `<div class="logo-box">YSQ SMART AI</div>`}
            <h1 class="main-title">Questionário de Esquemas de Young (YSQ-S3)</h1>
        </header>
        ...
        <div class="section">
            ...
            <div class="id-card">
                ...
                <div class="info-field">
                    ...
                    <span class="info-value">${formattedDate} ${formattedTime.replace('-', ':')}</span>
                </div>
            </div>
        </div>

        <div class="section">
            <div class="section-header">Mapeamento Quantitativo de Esquemas</div>
            <table>
                <thead>
                    <tr>
                        <th>Esquema Inicial Desadaptativo (EID)</th>
                        <th>Domínio Temático</th>
                        <th style="text-align: center; width: 80px;">Média (1-6)</th>
                        <th style="text-align: center; width: 140px;">Status Clínico</th>
                    </tr>
                </thead>
                <tbody>
                    ${tableRows}
        ...
        ${assessment.aiAnalysis ? `
          <div class="section" style="page-break-before: always;">
              <div class="section-header">Análise Clínico-Qualitativa (IA)</div>
        ...
        <footer>
            ...
            <div class="legal-notice">
                <strong>REFERÊNCIAS E RESPONSABILIDADES:</strong> O presente laudo técnico foi estruturado de acordo com as normas éticas estabelecidas pela Resolução CFP nº 06/2019. O Questionário de Esquemas de Young (YSQ-S3) é um inventário clínico autoaplicável de uso restrito a profissionais de psicologia no âmbito de avaliações psicológicas e psicodiagnósticos. A pontuação média reflete a autopercepção cognitiva atual e deve ser contextualizada clinicamente pelo terapeuta responsável.
            </div>
        </footer>
    </div>
