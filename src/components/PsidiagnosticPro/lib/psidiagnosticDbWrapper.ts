...
export function formatPsidiagnosticToHtml(record: DiagnosticRecord): string {
  return `
    <div class="psidiagnostic-entry-rendered p-6 bg-white/[0.01] border border-white/[0.06] rounded-2xl space-y-4">
      <div class="flex items-center justify-between border-b border-white/[0.08] pb-3 mb-3">
        <h4 class="text-xs font-black uppercase tracking-wider text-[#a78bfa]">Psidiagnostic Pro</h4>
      ...
      <p class="text-xs leading-relaxed opacity-95">
        Avaliação psicodiagnóstica realizada. Fontes de dados analisadas:
        ${record.hasProntuarioData ? '<strong>Histórico do Prontuário</strong>' : ''} 
        ${record.hasProntuarioData && record.uploadedFilesCount > 0 ? ' e ' : ''}
        ${record.uploadedFilesCount > 0 ? `<strong>${record.uploadedFilesCount} arquivo(s) anexo(s)</strong>` : ''}.
      </p>
      ${record.aiAnalysis ? `
        <div class="mt-4 pt-4 border-t border-white/[0.08]">
          <h5 class="text-xs font-black uppercase text-purple-400 tracking-wider mb-2">Conclusão do Laudo Psicodiagnóstico</h5>
...
class PsidiagnosticDatabaseWrapper {
  async getHistory(patientId?: string): Promise<DiagnosticRecord[]> {
    const assessments: DiagnosticRecord[] = [];
    if (patientId) {
      const record = await db.prontuarios.get(patientId);
      if (record) {
        record.entradas.forEach(e => {
          if (e.tipo === 'psidiagnostic' || e.metadata?.type === 'psidiagnostic') {
            const diag = e.metadata?.psidiagnosticData;
            if (diag) assessments.push(diag);
          }
        });
      }
    } else {
          ...
          if (e.tipo === 'psidiagnostic' || e.metadata?.type === 'psidiagnostic') {
            ...
            if (diag) {
              assessments.push({
                ...
                patient: {
                  ...diag.patient,
                  name: patient?.nome || diag.patient.name
                }
              });
            }
          }
  ...
  async deleteEntry(id: string, patientId: string, userId?: string): Promise<DiagnosticRecord[]> {
    const record = await db.prontuarios.get(patientId);
    if (record) {
...
export const dbWrapper = new PsidiagnosticDatabaseWrapper();
