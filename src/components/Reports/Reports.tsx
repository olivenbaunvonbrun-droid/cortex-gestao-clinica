...
export default function Reports() {
  ...
  const [activeTab, setActiveTab] = useState<'indicators' | 'schedule'>('indicators');
  ...
  const loadStats = async () => {
    ...
    // Filtramos apenas pacientes de status ativo ou não configurados (ativos padrão)
    ...
    // Guardando nas listas de controle do relatório
    setPatients(activePatients);
    setAppointments(listAppointments);

    // Carregar configurações de agenda / carga horária
    ...
    // Se houver pacientes e não há paciente selecionado prévio, seleciona o primeiro
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthlyIncome = transactions
        .filter(t => {
          ...
          return td.getMonth() === m && td.getFullYear() === y && t.tipo === 'receita';
        })
      ...
      const monthlyExpense = transactions
        .filter(t => {
          const td = new Date(t.data + 'T00:00:00');
          return td.getMonth() === m && td.getFullYear() === y && t.tipo === 'despesa';
        })
        .reduce((acc, t) => acc + t.valor, 0);
  ...
  const generateInformativeText = () => {
    ...
    const getWeekDays = (refDate: Date) => {
      ...
      const diff = d.getDate() - dayOfWeek; // local Sunday of that week
    ...
    // Gerar TIME_SLOTS dinamicamente com base nas configurações
    ...
    for (let h = startH; h < endH; h++) {
      if (hasLunchBreak && h >= lunchSH && h < lunchEH) {
        continue;
      }
      const hhStr = String(h).padStart(2, '0') + ':00';
      TIME_SLOTS.push(hhStr);
    }
    ...
    if (viewType === 'day') {
      ...
      if (reportType === 'scheduled' || reportType === 'both') {
        ...
        if (patientSessions.length > 0) {
          ...
          patientSessions.forEach(a => {
            text += `  • Horário: ${a.hora} (${a.tipo === 'online' ? 'Online' : 'Presencial'})\n`;
          });
          text += `\n`;
        }
      }
      ...
      if (reportType === 'availability' || reportType === 'both') {
        const freeSlots = TIME_SLOTS.filter(slot => {
          return !appointments.some(a => a.data === dateStr && a.hora === slot && a.status !== 'cancelled');
        });
        ...
        if (freeSlots.length > 0) {
          ...
          text += `  • Sem horários livres disponíveis para este dia.\n`;
        }
      }
      ...
      if (reportType === 'scheduled' || reportType === 'both') {
        ...
        weekDays.forEach(day => {
          ...
          if (patientSessions.length > 0) {
            ...
            patientSessions.forEach(a => {
              sessionLines += `  • Às ${a.hora} (${a.tipo === 'online' ? 'Online' : 'Presencial'})\n`;
            });
          }
        });

        if (hasSessions) {
          text += `📌 Suas Sessões Agendadas nesta semana:\n` + sessionLines + `\n`;
        }
      }
      ...
      if (reportType === 'availability' || reportType === 'both') {
        ...
        weekDays.forEach(day => {
          ...
          if (freeSlots.length > 0) {
            text += `  • ${getDayNamePT(day)} (${String(day.getDate()).padStart(2, '0')}/${String(day.getMonth() + 1).padStart(2, '0')}): ${freeSlots.join(', ')}\n`;
          }
        });
      }
      ...
      if (reportType === 'availability' || reportType === 'both') {
        ...
        weekDays.forEach(day => {
          ...
          if (freeSlots.length > 0) {
            text += `  • Nos dias de ${getDayNamePT(day)}s: ${freeSlots.slice(0, 3).join(', ')}...\n`;
          }
        });
      }
    }
  ...
  const handleShareWhatsApp = () => {
    ...
    const message = encodeURIComponent(customInformativo);
  ...
  const handleShareEmail = () => {
    ...
    const text = encodeURIComponent(customInformativo);
    const subject = encodeURIComponent('Informativo de Agenda e Disponibilidade');
    window.open(`mailto:${patientObj.email}?subject=${subject}&body=${text}`, '_blank');
  };
    ...
    datasets: [
      {
        ...
        backgroundColor: 'rgba(191, 155, 107, 0.4)', // Using a shade closer to our primary gold
  ...
  const chartOptions = {
    ...
    plugins: {
      legend: {
        ...
        labels: { 
          color: '#808080',
          font: { family: 'Space Grotesk', weight: '700' as any, size: 10 },
          boxWidth: 10,
          usePointStyle: true,
          padding: 20
        }
    ...
    scales: {
      y: { 
        ...
        ticks: { color: '#4D4D4D', font: { size: 10, family: 'JetBrains Mono' } } 
      },
      x: { 
        grid: { display: false }, 
        ticks: { color: '#4D4D4D', font: { size: 10, family: 'JetBrains Mono' } } 
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex border-b border-border-subtle gap-8 pb-px">
        <button
          onClick={() => setActiveTab('indicators')}
          className={cn(
            "pb-4 text-xs font-black uppercase tracking-[0.2em] transition-all relative outline-none",
      ...
      {activeTab === 'indicators' ? (
        <>
          {/* Aba 1: Indicadores BI originais */}
        ...
        /* Aba 2: Relatório de Agenda & Disponibilidade */
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in duration-500">
            ...
            <div className="bg-bg-card border border-border-subtle p-8 rounded-[2.5rem] shadow-2xl space-y-6 relative overflow-hidden">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <button
                      key={t.id}
                      onClick={() => setViewType(t.id as any)}
                      className={cn(
                        "py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all text-center",
                        viewType === t.id
                          ? "bg-primary text-bg-deep shadow-lg shadow-primary/10"
                          : "bg-bg-sidebar/60 border border-border-subtle text-text-dim hover:text-text-main"
                      )}
                    >
                      {t.label}
                    </button>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between bg-bg-sidebar border border-border-subtle rounded-2xl p-2.5">
                  <button
                    onClick={() => handleNavigateReference('prev')}
                    className="p-2 hover:bg-white/5 rounded-xl text-text-dim hover:text-text-main transition-all border border-border-subtle/50"
                  >
                    <ChevronLeft size={16} />
                  </button>
                  ...
                  <button
                    onClick={() => handleNavigateReference('next')}
                    className="p-2 hover:bg-white/5 rounded-xl text-text-dim hover:text-text-main transition-all border border-border-subtle/50"
                  >
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  {[
                    { id: 'both', label: 'Agenda + Horários Livres' },
                    { id: 'scheduled', label: 'Apenas Meus Agendamentos' },
                    { id: 'availability', label: 'Apenas Horários Livres' }
                  ].map(r => (
                    <button
                      key={r.id}
                      onClick={() => setReportType(r.id as any)}
                      className={cn(
                        "w-full text-left px-5 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border block",
                        reportType === r.id
            ...
            <div className="bg-bg-card border border-border-subtle p-8 rounded-[2.5rem] shadow-2xl space-y-6 relative overflow-hidden">
              ...
              <div className="space-y-4 max-h-[380px] overflow-y-auto pr-1">
                {(() => {
                  ...
                  if (viewType === 'day') {
                    daysToRender = [referenceDate];
                  } else if (viewType === 'week') {
                    daysToRender = getWeekDays(referenceDate);
                  } else if (viewType === 'month') {
                    const year = referenceDate.getFullYear();
                    ...
                    return (
                      <div key={dStr} className="p-4 bg-bg-sidebar/50 rounded-2xl border border-border-subtle/50 space-y-3">
                        <div className="flex justify-between items-center border-b border-border-subtle/40 pb-2">
                          <span className="text-[10px] font-bold text-text-main uppercase tracking-wider">{getDayNamePT(day)}</span>
                          ...
                          {TIME_SLOTS.map(slot => {
                            ...
                            return (
                              <div 
                                key={slot} 
                                className={cn(
                                  "py-2 px-0.5 rounded-xl text-center font-mono transition-all border flex flex-col justify-between h-9",
                                  isPatientAppt 
                                    ? "bg-primary text-bg-deep border-primary" 
                                    : isBooked 
                                      ? "bg-red-500/10 text-red-400 border-red-500/20" 
                                      : "bg-green-500/5 text-green-400 border-green-500/25"
                                )}
                                title={isPatientAppt ? 'Agendado com este paciente' : isBooked ? 'Ocupado com outro paciente' : 'Livre'}
                              >
          ...
          {/* Coluna Direita: Editor do Informativo / Envio */}
              ...
              <div className="border-t border-border-subtle/50 pt-8 mt-8">
                  ...
                  return (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <button
                        ...
                        disabled={!selectedPatientId || !hasPhone}
                        className={cn(
                          "flex items-center justify-center gap-3 py-4.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg",
                          selectedPatientId && hasPhone
                            ? "bg-green-500 text-white hover:bg-green-600 shadow-green-500/10 hover:scale-[1.01]"
                            : "bg-border-subtle text-text-dim cursor-not-allowed opacity-45"
                        )}
                      >
                      ...
                      <button
                        ...
                        disabled={!selectedPatientId || !hasEmail}
                        className={cn(
                          "flex items-center justify-center gap-3 py-4.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg",
                          selectedPatientId && hasEmail
                            ? "bg-primary text-bg-deep hover:bg-white shadow-primary/10 hover:scale-[1.01]"
                            : "bg-border-subtle text-text-dim cursor-not-allowed opacity-45"
                        )}
                      >
                ...
                {(() => {
                  ...
                  if (selectedPatientId && patientObj && !patientObj.telefone && !patientObj.email) {
                    return (
                      <p className="text-[9px] text-red-400 font-bold uppercase tracking-wider text-center mt-4">
                        Aviso: Este paciente selecionado não possui e-mail nem telefone cadastrados.
                      </p>
                    );
                  }
                  return null;
